...
func TestTreeAddAndGet(t *testing.T) {
	...
	for _, route := range routes {
		tree.addRoute(route, HandlersChain{fakeHandler(route)})
	}
...
func TestTreeRootTrailingSlashRedirect(t *testing.T) {
	...
	recv := catchPanic(func() {
		tree.addRoute("/:test", HandlersChain{fakeHandler("/:test")})
	})
	if recv != nil {
...
func TestTreeInvalidNodeType(t *testing.T) {
	...
	tree := &node{}
	tree.addRoute("/", HandlersChain{fakeHandler("/")})
	tree.addRoute("/:page", HandlersChain{fakeHandler("/:page")})
...
func TestTreeWildcardConflictEx(t *testing.T) {
	...
	for i := range conflicts {
		...
		recv := catchPanic(func() {
			tree.addRoute(conflict.route, HandlersChain{fakeHandler(conflict.route)})
		})
...
func TestRedirectTrailingSlash(t *testing.T) {
	...
	for _, item := range data {
		node.addRoute(item.path, HandlersChain{fakeHandler("test")})
	}
